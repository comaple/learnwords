...
class MemoryService:
    ...
    def due_for_user(self, db: Session, user_id: str, limit: int = 50):
        now = datetime.utcnow()
        rows = db.query(models.UserWord).filter(models.UserWord.user_id==user_id, models.UserWord.next_review_at<=now).order_by(models.UserWord.next_review_at).limit(limit).all()
        return rows
